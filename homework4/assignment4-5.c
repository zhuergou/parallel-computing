...
#include</gpfs/u/home/PCP7/PCP7llll/assign4/clcg4.h>
#include<pthread.h>
#include<mpi.h>
...
*/
	...
	int realCHILDREN = (CHILDREN_test + 1)/mpi_commsize - 1;
	if(mpi_commsize == 16)
        realCHILDREN = 8;
	//printf("CHILDER test : %d\n", realCHILDREN);

	// number of row for each thread
	int row_per_thread = row_per_rank/(realCHILDREN + 1);

	// if it has children, then put it into the new thread
	if (realCHILDREN != 0){
		for(j = 0; j < realCHILDREN; j++){
			variable *t;
	...
	for (i = 0; i < TICK; i++){
		...
		while (1){
			if (semaphore1 %(realCHILDREN + 1) == 0){
				break;
			}
		}
		...
		pthread_mutex_unlock(&mutex);
		while (1){
			if (semaphore2 %(realCHILDREN + 1) == 0){
				break;
			}
		}
		...
		pthread_mutex_unlock(&mutex);
		while (1){
			if (semaphore3 %(realCHILDREN + 1) == 0){
				break;
			}
		}
...
void * whattodo(void *arg){
	...
	int realCHILDREN = arg_temp -> realCHILDREN;
